...
import { Award, ScrollText, Handshake, Heart, Eye, ArrowRight, GraduationCap, Trophy, Globe2, Sparkles } from "lucide-react";
...
export default function NosotrosPage() {
  return (
    <Layout>
      <PageHero eyebrow="Sobre nosotros" title="Un bufete construido sobre principios" subtitle="Conoce la historia, los valores y al equipo que ha defendido a miles de clientes salvadoreños durante más de dos décadas." />
      <section className="py-24 md:py-32 bg-gradient-cream">
        <div className="mx-auto max-w-7xl px-6 grid gap-16 lg:grid-cols-2 items-center">
          <Reveal direction="left">
            ...
            <h2 className="font-display text-4xl md:text-5xl text-navy leading-tight">Dos décadas defendiendo lo que <em className="text-gold-deep">más importa</em>.</h2>
            <span className="gold-divider mt-6 block" />
            <div className="mt-7 space-y-4 text-muted-foreground leading-relaxed text-lg">
              <p>Vargas & Asociados nació en 2002 con una convicción simple: <span className="text-navy font-medium">el derecho debe estar al servicio de las personas</span>. Lo que comenzó como un pequeño despacho con tres abogados se ha convertido en una firma de referencia en El Salvador, con más de doce profesionales especializados.</p>
              <p>A lo largo de los años hemos representado a familias, emprendedores, empresas medianas y grandes corporaciones. Cada cliente es un caso, una historia, una responsabilidad que asumimos con la seriedad que merece.</p>
              <p>Hoy seguimos creciendo, pero conservamos el espíritu fundacional: trato cercano, estrategias claras y resultados que respalden nuestra reputación a nivel regional.</p>
            </div>
          </Reveal>
          <Reveal direction="right" className="relative">
            <motion.img whileHover={{ scale: 1.02 }} transition={{ duration: 0.6 }} src={aboutImg} alt="Símbolo de la justicia" loading="lazy" width={1024} height={1024} className="w-full aspect-[4/5] object-cover shadow-elegant" />
            <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7, delay: 0.3 }} className="absolute -bottom-8 -left-8 hidden md:block bg-gold text-navy-deep p-7 max-w-xs shadow-elegant">
          ...
          <StaggerGroup className="grid grid-cols-2 md:grid-cols-4 gap-10">
            {stats.map((s) => (
              <StaggerItem key={s.l} className="text-center">
                <div className="font-display text-5xl md:text-7xl text-gradient-gold"><Counter to={s.v} suffix={s.s} /></div>
            ...
            {milestones.map((m, i) => (
              <Reveal key={m.year} direction={i % 2 === 0 ? "left" : "right"} delay={i * 0.05}>
                <div className={`relative pl-20 md:pl-0 md:grid md:grid-cols-2 md:gap-12 mb-14 items-center ${i % 2 === 0 ? "" : "md:[&>div:first-child]:order-2"}`}>
                  <motion.div initial={{ scale: 0 }} whileInView={{ scale: 1 }} viewport={{ once: true }} transition={{ type: "spring", delay: 0.2 }} className="absolute left-8 md:left-1/2 top-2 md:top-1/2 -translate-x-1/2 md:-translate-y-1/2 h-5 w-5 bg-gold rounded-full ring-4 ring-background shadow-gold" />
          ...
          <StaggerGroup className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {values.map((v) => (
              <StaggerItem key={v.title}>
                <motion.div whileHover={{ y: -8 }} className="group bg-white border border-border p-10 hover:border-gold transition-all hover:shadow-elegant duration-500 h-full">
                  <motion.div whileHover={{ rotate: 10, scale: 1.1 }} className="flex h-14 w-14 items-center justify-center bg-gradient-gold mb-6 shadow-gold group-hover:shadow-elegant">
      ...
      <section className="py-24 md:py-32 bg-background">
        <div className="mx-auto max-w-7xl px-6">
          <Reveal className="text-center max-w-2xl mx-auto mb-20">
            ...
            <span className="gold-divider mt-7" />
            <p className="mt-7 text-muted-foreground text-lg">Un equipo multidisciplinario unido por la misma vocación: defender con excelencia.</p>
          </Reveal>
              ...
              <StaggerItem key={p.name}>
                <motion.div whileHover={{ y: -8 }} className="group">
                  <div className="relative overflow-hidden aspect-[3/4] shadow-soft">
                    <motion.img whileHover={{ scale: 1.08 }} transition={{ duration: 0.7 }} src={p.img} alt={`${p.name}, ${p.role}`} loading="lazy" width={800} height={1067} className="h-full w-full object-cover" />
          ...
          <Reveal delay={0.2} className="mt-16">
            <div className="bg-navy-deep text-white p-12 md:p-16 grid md:grid-cols-3 gap-10 items-center shadow-elegant">
              <div className="md:col-span-2">
                ...
                <p className="mt-4 text-white/70 leading-relaxed">Mantenemos convenios con bufetes en Centroamérica, México, Estados Unidos y España, lo que nos permite acompañar a nuestros clientes más allá de las fronteras de El Salvador.</p>
              </div>
      ...
      <section className="py-24 bg-gradient-navy text-white relative overflow-hidden grain">
        <div className="relative mx-auto max-w-4xl px-6 text-center">
          <Reveal>
            ...
            <a href="https://wa.me/50378900000" target="_blank" rel="noopener" className="group mt-10 inline-flex items-center gap-3 btn-gold px-9 py-5 text-xs font-semibold uppercase tracking-[0.22em]">
